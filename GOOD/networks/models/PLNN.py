...
class PL_GIN(GNNBasic):
    ...
    def __init__(self, config: Union[CommonArgs, Munch]):
        ...
        self.max_k = config.ood.extra_param[3]
    ...
    def forward(self, *args, **kwargs) -> Tuple:
        if self.training:
            ...
            for pair_id in range(num_pair):
                # out_o = out_readout[batch == pair_id]
                # if self.config.model.global_pool == 'mean':
                #     org_out.append(out_o.mean(0))
                # else:
                #     org_out.append(out_o.max(0)[0])
                ...
                if self.stochastic:
                    ...
                    size_a = torch.randint(max(int(0.5 * cos_sim.shape[0]), 1), max(int(0.8 * cos_sim.shape[0]), 2), ()).item()
                    k = min(size_a, self.max_k)
                    # k = min(math.ceil(cos_sim.shape[0] * self.ratio), self.max_k)
            ...
            batch = data.batch
            out_readout = self.feature_extractor(*args, **kwargs)
            out_readout = self.readout(out_readout, batch, batch[-1] + 1)
            # matched_out = []
        ...
        if self.training:
            ...
            for pair_id in range(num_pair):
                ...
                if self.config.model.global_pool == 'mean':
                    ...
                    org_out.append(out_o.max(0)[0])
                out_a = out_readout[batch == (pair_id + num_pair)]
                out_b = out_readout[batch == (pair_id + 2*num_pair)]
                if out_a.shape[0] > out_b.shape[0]:
                    out_a, out_b = out_b, out_a
                ...
                if self.stochastic:
                    ...
                    max_a, pick_b = cos_sim.max(1)
                    size_a = torch.randint(int(0.5 * cos_sim.shape[0])+1, int(0.8 * cos_sim.shape[0]) + 2, ()).item()
                    k = min(size_a, self.max_k)
                    # k = min(math.ceil(cos_sim.shape[0] * self.ratio), self.max_k)
                    pick_a = torch.topk(max_a, k, dim=0).indices
                    # if pick_a.sum() == 0:
            ...
            matched_out = torch.stack((org_out + matched_out), 0)
            out = self.classifier(matched_out)
        else:
            # data = kwargs.get('data')
            # batch = data.batch
