...
class SODAug(BaseOODAlg):

    def __init__(self, config: Union[CommonArgs, Munch]):
        super(SODAug, self).__init__(config)
        self.lam = None
        self.data_perm = None
        self.id_a2b: Tensor
        self.id: Tensor
    ...
    def input_generation(self,
        ...
        if training:  # and config.train.epoch > 20:
            ...
            for idx_a, idx_b in enumerate(self.id_a2b):
                ...
                if idx_a+1 > config.ood.extra_param[10]*int(batch_size/2):
                    if hasattr(data, 'edge_attr') and getattr(data, 'edge_attr') is not None:
                        ...
                        org_batch.append(
                            Data(x=data_a.x, edge_index=data_a.edge_index, y=data_a.y, env_id=data_a.env_id))
                    continue
                ...
                if hasattr(data, 'edge_attr') and getattr(data, 'edge_attr') is not None:
                    ...
                    frag_batch.append(Data(x=x_f, edge_index=edge_idx_f, edge_attr=edge_attr_f, y=data_a.y,env_id=env_frag))
                    if config.ood.extra_param[9] > 0.5:
                        recom_batch.append(
                    ...
                    frag_batch.append(Data(x=x_f, edge_index=edge_idx_f, y=data_a.y, env_id=env_frag))
                    if config.ood.extra_param[9] > 0.5:
            ...
            for idx_ in range(batch_size):
                ...
                if hasattr(data, 'edge_attr') and getattr(data, 'edge_attr') is not None:
                    final_batch.append(Data(x=data_a.x, edge_index=edge_idx, edge_attr=edge_attr, y=data_a.y, env_id=env_list[idx_][None]))
                else:
                    final_batch.append(Data(x=data_a.x, edge_index=edge_idx, y=data_a.y, env_id=env_list[idx_][None]))
