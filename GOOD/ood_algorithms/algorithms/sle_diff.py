...
class G_splice_diff(BaseOODAlg):
    r"""
    ...
    def __init__(self, config: Union[CommonArgs, Munch]):
        super(G_splice_diff, self).__init__(config)
        self.lam = None
        self.data_perm = None
        self.id_a2b: Tensor
        self.id: Tensor
    ...
    def input_generation(self,
        ...
        if training:  # and config.train.epoch > 20:
            ...
            for idx_a, idx_b in enumerate(self.id_a2b):
                ...
                # y = 0
                noise1 = torch.randn((data_a.x.shape[0], data_b.x.shape[0]), device=config.device)

                if config.dataset.dataset_type == 'mol':
                    new_batch.append(Data(x=x, edge_index=edge_idx, edge_attr=edge_attr, z_t=noise1.reshape(-1), frag_1=data_a.x.shape[0],
                                          frag_2=data_b.x.shape[0], y=data_a.y))
                    org_batch.append(
                        Data(x=data_a.x, edge_index=data_a.edge_index, edge_attr=data_a.edge_attr, y=data_a.y,
                             env_id=data_a.env_id))
                else:
                    new_batch.append(
                        Data(x=x, edge_index=edge_idx, z_t=noise1.reshape(-1), frag_1=data_a.x.shape[0], frag_2=data_b.x.shape[0], y=data_a.y))
            ...
            num_bridge_all = torch.round(model_output[2]).long()
            ...
            for idx_ in range(batch_size):
                ...
                if config.dataset.dataset_type == 'mol':
                    if num_bridge > 0:
                        bridge_attr_pred = model_output[1].detach()
                        # bridge_attr_idx = torch.randint(0, data_a.edge_attr.shape[0], (1, num_bridge), device=config.device)
                        # bridge_attr = torch.squeeze(copy.deepcopy(data_a.edge_attr[bridge_attr_idx]), dim=0)
                ...
                if config.dataset.dataset_type == 'mol':
                    final_batch.append(
                        Data(x=data_a.x, edge_index=edge_idx, edge_attr=edge_attr, y=data_a.y, env_id=env_new))
                else:
